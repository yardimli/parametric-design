...
if (is_null($userId)) {
	...
	if (app('auth.oauth')->signup($provider, $data)) {
		app('auth.oauth')->login($provider, $user['id']);
	}
} else {
	app('auth.oauth')->login($provider, $user['id']);
}

if (app('auth.oauth')->passes()) {
	...
	<div class="col-md-6">
		...
		<?php if (is_null($userId)): ?>
			<form action="" method="POST" class="clearfix">
				...
				<?php if (Config::get('auth.require_username') && app('auth.oauth')->errors()->has('username')): ?>
				...
				<?php if (app('auth.oauth')->errors()->has('email')): ?>
				    <div class="form-group">
				...
				<?php foreach (UserFields::with((array) $data['usermeta'])->all('signup') as $key => $f): ?>
					<?php if (app('auth.oauth')->errors()->has($key)): ?>
